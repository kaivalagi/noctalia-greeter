...
namespace {
  ...
  void writeSyslog(std::string_view level, std::string_view message) {
    ...
    syslog(priority, "[%.*s] %.*s", static_cast<int>(level.size()), level.data(),
...
void emergencyLogBootstrap(int argc, char* argv[]) {
  ...
  msg << "=== noctalia-greeter bootstrap pid=" << ::getpid() << " uid=" << ::getuid()
      << " euid=" << ::geteuid();
  if (exeLen > 0) {
    msg << " exe=" << exePath;
  }
...
void initLogging() {
  ...
  if (!g_logPaths.empty()) {
    ...
    std::fprintf(stderr,
                 "[warn] [log] no log file opened — check permissions on /var/lib/noctalia-greeter "
                 "and /var/log/noctalia-greeter.log\n");
    std::fflush(stderr);
  }
}

const char* loggingPaths() {
  return g_logPaths.c_str();
}
...
void Logger::log(std::string_view level, std::string_view message) const {
  writeLogLine(m_tag, level, message);
}
