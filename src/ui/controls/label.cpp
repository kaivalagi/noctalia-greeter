...
bool Label::setText(std::string_view text) {
  if (m_plainText == text) return false;
...
void Label::setFontSize(float size) {
  if (m_textNode->fontSize() == size) return;
  m_textNode->setFontSize(size);
  m_measureCached = false;
}

void Label::setFontFamily(std::string family) {
  if (m_textNode->fontFamily() == family) return;
...
void Label::setMinWidth(float minWidth) {
  if (m_minWidth == minWidth) return;
  m_minWidth = minWidth;
  m_measureCached = false;
}

void Label::setMaxWidth(float maxWidth) {
  if (m_userMaxWidth == maxWidth) return;
...
void Label::setMaxLines(int maxLines) {
  if (m_userMaxLines == maxLines) return;
...
void Label::setBold(bool bold) {
  if (m_textNode->bold() == bold) return;
  m_textNode->setBold(bold);
  m_measureCached = false;
}

void Label::setTextAlign(TextAlign align) {
  if (m_textNode->textAlign() == align) return;
...
LayoutSize Label::measureWithConstraints(Renderer& renderer, const LayoutConstraints& constraints) {
  ...
  auto metrics = renderer.measureText(m_plainText, m_textNode->fontSize(), m_textNode->bold(),
                                       measureMaxWidth, m_userMaxLines, m_textNode->textAlign(),
  ...
  float w = constraints.hasExactWidth() ? std::max(constraints.maxWidth, m_minWidth)
                                         : std::max(measuredWidth, m_minWidth);
...
void Label::doLayout(Renderer& renderer) {
  measure(renderer);
}
